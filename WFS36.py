...
def WFS_dep():  # Calculates x offsets at WFS1 and WFS2 for +/- dislpacement/direction errors at SRM.
        ...
        WFS_plot2(Stest[0],Stest[1],Stest[2],Stest[3])
...
def WFS_orthogonality2(x1_displ,x2_displ,x1_direc,x2_direc): # Calculates phase-separation between WFS which sense displacement and direction errors upstream.
    v1 = np.array([[x1_direc[-1] - x1_direc[0], x1_displ[-1] - x1_displ[0]]])
    v2 = np.array([[x2_direc[-1] - x2_direc[0]], [x2_displ[-1] - x2_displ[0]]])
    v1_norm = v1 / np.sqrt(v1[0,0]**2 + v1[0,1]**2)
    v2_norm = v2 / np.sqrt(v2[0,0]**2 + v2[1,0]**2)
    dot_product = np.dot(v1_norm,v2_norm)
    phi_ASC = (180 / np.pi) * np.arccos(dot_product)
...
def WFS_plot2(x1_displ,x2_displ,x1_direc,x2_direc,n=4): # Plots displacement at WFS1 vs displacement at WFS2 when displacement and direction errors are applied at the SRM. 
    plt.figure(n, figsize=(6, 5.5), dpi=120)
    plt.plot(x1_direc, x1_displ, color = 'blue', label = 'WFS1')                        
    plt.plot(x2_direc, x2_displ, color = 'orange', label = 'WFS2')                         
    plt.title('Apply Pure Displacement and Direction errors at Waist 3')         
    plt.legend(loc = 'upper right')
    axes = plt.gca()
    axes.set_xlim([-3.5, 3.5])
    ...
    textstr = 'Orthogonality: %.1f˚' % (WFS_orthogonality2(x1_displ,x2_displ,x1_direc,x2_direc),)
    props = dict(boxstyle='square', facecolor='white', alpha=0.5)
    axes.text(0.02, 0.98, textstr, transform=axes.transAxes, fontsize=10,verticalalignment='top', bbox=props)
    plt.tight_layout()
